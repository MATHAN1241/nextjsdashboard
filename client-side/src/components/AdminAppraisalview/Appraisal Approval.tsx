...
const ApprovalAppraisal = () => {
  ...
  const [showApprovedAppraisalBox, setShowApprovedAppraisalBox] = useState(false);
  const [showApprovedAppraisalFormBox, setShowApprovedAppraisalFormBox] = useState(false);
  ...
  const handleApprovalLinkClick = (event: { preventDefault: () => void; }) => {
    ...
    setShowApprovedAppraisalBox(false);
    setShowApprovedAppraisalFormBox(false);
  };
  ...
  const handleApprovedAppraisalLinkClick = (event: { preventDefault: () => void; }) => {
    ...
    setShowApprovedAppraisalBox(false);
    setShowApprovedAppraisalFormBox(true);
    // setShowApprovalBox(false);
    // setShowApprovedAppraisalBox(true);
    // setShowApprovedAppraisalFormBox(false);
  };
  
  const handleApprovedAppraisalFormLinkClick = (event: { preventDefault: () => void; }) => {
    event.preventDefault();
    // setShowApprovalBox(false);
    // setShowApprovedAppraisalBox(false);
    // setShowApprovedAppraisalFormBox(true);
  ...
  const handleNoRecordsFoundClick = () => {
        ...
        <div className="grid  md:grid-row items-center mb-4">
          <div className="bg-white border border-gray-300 rounded-lg p-4 mb-4 w-full md:w-auto md:mr-4 text-black ">
            <div>
              ...
              <a
                className={`pl-5 ${showApprovedAppraisalBox ? 'text-violet-600' : 'text-gray-600'} font-bold hover:text-violet-700`}
              ...
              <a
                className={`pl-5 ${showApprovedAppraisalFormBox ? 'text-violet-600' : 'text-gray-600'} font-bold hover:text-violet-700`}
                onClick={ handleApprovedAppraisalFormLinkClick }
              >
              Appraisal pending for previous level approver
              </a>
            </div>
            ...
            {showApprovalBox && (
              <div className="grid grid-cols-12 bg-gray-200 p-4 rounded-lg shadow-md co ">
                <h2 className=" text-lg font-bold mb-2 col-span-12">Waiting for Approval</h2>
              </div>
            )}

            {showApprovedAppraisalBox && (
              // Add content for approved appraisal box here
              <div className='grid grid-cols-12 bg-gray-200 p-4 rounded-lg shadow-md co'>
                 <h2 className=" text-lg font-bold mb-2 col-span-12">No records found</h2>
              </div>
            )}

            {showApprovedAppraisalFormBox && (
              <div className="bg-white border border-gray-300 rounded-lg p-4 mb-4">
                {records.length > 0 ? (
                  ...
                  <div onClick={handleNoRecordsFoundClick}>
...
export default ApprovalAppraisal;
